    ...
    <section className="bg-ibk-dark-surface py-28 px-4 sm:px-6 lg:px-8 border-t border-white/10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-16">
          ...
          <FadeIn delay={0.1}>
            <Link href="/novidades" className="flex items-center gap-2 text-white/65 hover:text-white font-display font-bold text-sm transition-colors group shrink-0">
          ...
          <FadeIn className="lg:col-span-7">
            <Link href={`/novidades/${destaque.slug}`} className="group block relative overflow-hidden rounded aspect-[16/10] bg-ibk-dark-surface">
              ...
              <div className="absolute inset-0 z-10 flex flex-col justify-end p-8">
                ...
                <h3 className="font-display font-extrabold text-2xl sm:text-3xl text-white leading-tight mb-2 group-hover:text-white/90 transition-colors">
                  {destaque.titulo}
                </h3>
          ...
          <div className="lg:col-span-5 flex flex-col gap-4">
            {resto.slice(0, 3).map((n, i) => (
              <FadeIn key={n.slug} delay={i * 0.08} direction="left">
                <Link href={`/novidades/${n.slug}`} className="group flex gap-4 p-4 rounded border border-white/10 bg-ibk-dark-card hover:border-[#E84C1E]/40 hover:bg-[#2e1a0e] transition-all">
                  <div
