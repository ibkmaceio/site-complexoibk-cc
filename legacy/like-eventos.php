...
$verifica = mysql_query("SELECT * FROM _eventos_like WHERE  ip =  '$ip'")
or die(mysql_error());
...
if($verifica > 0){
  ...
  */
  ...
  $query = mysql_query("SELECT slug FROM _eventos_like WHERE slug = '".$slug. "'");
  $res = mysql_fetch_array($query);
  $slug = $res['slug'];
  $query2 = mysql_query("UPDATE _eventos_like SET ip = '', like_s = '0' WHERE slug = '".$slug."'");

  /*
  VERIFICA O TOTAL DE LIKES
  */

  $soma_likes = mysql_query("SELECT  *,sum(like_s) as total_likes FROM _eventos_like WHERE slug = '".$_POST['slug']."' ");
  ...
  */
  $query_like = mysql_query ("INSERT INTO _eventos_like (slug,like_s,ip) VALUES ('".$slug."','".$like_s."','".$_SERVER['REMOTE_ADDR']."') ");

  if ($query_like) {
    $soma_likes_2 = mysql_query("SELECT  *,sum(like_s) as total_likes_2 FROM _eventos_like WHERE slug = '".$_POST['slug']."' ");
